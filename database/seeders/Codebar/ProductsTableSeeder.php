        ...
        $this->seed(
            ...
            localizedData: [
                LocaleEnum::DE->value => [
                    ...
                    'tags' => [],
                    'content' => <<<'MD'
                        Flows ist eine Orchestrierungsplattform für dokumentenbasierte Prozesse. Verbinde das Dokumentenmanagementsystem, das du bereits einsetzt, und lass KI-Agenten deine Dokumente automatisch extrahieren, validieren und verarbeiten – in einer isolierten Umgebung deiner Wahl.
                        ...
                        ## Orchestrierte Agenten-Workflows – extrahieren, verstehen, validieren, zurückschreiben.

                        Flows verbindet sich mit deinem bestehenden DMS. Keine Migration; dein Archiv bleibt, wo es ist. Darauf konfigurierst du Workflows, die auslösen, sobald ein Dokument eintrifft.

                        - **Agentische Extraktion mit Validierung**: Definiere das Schema, das du brauchst. Agenten extrahieren die Daten, validieren sie gegen deine eigenen Systeme und Daten und schreiben geprüfte Ergebnisse zurück – strukturiert und passend zu deinen Feldnamen.
                        - **Multi-Agenten-Workflow-Orchestrierung**: Für komplexe Prozesse verkettest du spezialisierte Agenten – Klassifizierung, Validierung, Anreicherung, Routing. Die Orchestrierung basiert auf dem Microsoft Agent Framework, und MCP-Tool-Integrationen verbinden deine Workflows direkt mit deinen Geschäftssystemen und Daten.
                        - **Prompt- und Agenten-Engineering**: Verbessere Prompts anhand echter Produktionsdaten, generiere sie aus dem Wissen in deinem Datenpool oder DMS und verwalte Prompts und Schemas in einer versionierten, wiederverwendbaren Knowledge-Bibliothek.
                        - **Modell- und Anbieterfreiheit**: Führe Workflows bei dem Anbieter aus, der zu deinen Anforderungen an Genauigkeit, Kosten und Datenresidenz passt – und vergleiche denselben Workflow über mehrere Modelle hinweg, um zu sehen, welches bei deinen Dokumenten am besten abschneidet. Ein Wechsel bedeutet keinen Neuaufbau. Eigene lokale Modelle (Bring-your-own) folgen als Nächstes.
                        - **Vollständige Nachvollziehbarkeit**: Jeder Lauf wird lückenlos protokolliert: Dokument, Modell, Tokens, Dauer, Ergebnis.
                        MD,
                ...
                LocaleEnum::EN->value => [
                    ...
                    'tags' => [],
                    'content' => <<<'MD'
                        Flows is an orchestration platform for document-based processes. Connect the document management system you already run, and let AI agents extract, validate, and process your documents automatically — in an isolated environment you choose.

                        ## Extraction tools read documents. They don't understand your business.

                        Two problems keep document processes manual. The first is effort: your DMS stores documents reliably, but the work around it — reading, indexing, validating, moving data into other systems — still depends on people opening files and typing. It only scales by hiring.
                        ...
                        ## Orchestrated agent workflows — extract, understand, validate, write back.

                        Flows connects to your existing DMS. No migration; your archive stays where it is. On top of it, you configure workflows that trigger the moment a document arrives.

                        - **Agentic extraction with validation**: Define the schema you need. Agents extract the data, validate it against your own systems and data, and write verified results back — structured and keyed to your field names.
                        - **Multi-agent workflow orchestration**: For complex processes, chain specialized agents — classification, validation, enrichment, routing. Orchestration is built on the Microsoft Agent Framework, and MCP tool integrations connect your workflows directly to your business systems and data.
                        - **Prompt & agent engineering**: Improve prompts from real production data, generate them from the knowledge in your data pool or DMS, and manage prompts and schemas in a versioned, reusable Knowledge library.
                        - **Model and provider freedom**: Run workflows on the provider that fits your accuracy, cost, and data-residency requirements — and benchmark the same workflow against multiple models to see which performs best on your documents. Switching doesn't mean rebuilding. Bring-your-own local models is next.
                        - **Full traceability**: Every run is logged end to end: document, model, tokens, duration, result.
                        MD,
